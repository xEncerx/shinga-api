...
<prompt>
  ...
  <output_format>
    Возвращайте ТОЛЬКО ТЕКСТ с переведенными полями, ВЫДЕЛЕННЫМИ КАВЫЧКИ.
    ЭКРАНИРУЙ ВСЕ НЕОБХОДИМЫЕ ДЛЯ ПЕРЕВОДА СТРОКИ В JSON СИМВОЛЫ
    БЕЗ ПОЯСНЕНИЙ, ЛИШНИХ СИМВОЛОВ и ФОРМАТИРОВАНИЯ:
    {{
